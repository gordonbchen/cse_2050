...
random.seed(
    652
)  # Fix the random seed so when a test fails, it fails the same way every time
...
class TestOrderedList(unittest.TestCase):
    ...
    def test_add(self):
        ...
        # Note how we parameterize the length. Reduce this to a small number (e.g. 5)
        n = 1000
        ...
        for i in range(n):
            # generate random int, ensure it's not already
            # in OL if this is the first time seeing it
            new_int = random.randint(0, 100)
            if new_int not in unsorted:
            ...
            # make sure count is the same in both collections
    ...
    def test_remove(self):
        ...
        for i in range(n):
            ...
            if item not in unsorted:  # contains
            ...
            # remove item from unordered list, make sure
            # count is the same in both collections
