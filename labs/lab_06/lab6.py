class OrderedList:
    def __init__(self, items=None):
        """
        Initialize an ordered list. If `items` is specified,
        the OrderedList starts with the items in that collection
        """
    ...
    def remove(self, item):
        ...
        Raise a RuntimeError if the item is not present.
        """
        if item not in self:
    ...
    def __getitem__(self, index):
        """
        returns the item with the given index in the sorted list.
    ...
    def __iter__(self):
        """
        returns an iterator over the ordered list that yields the items in sorted order.
    ...
    def __contains__(self, item):
        ...
        # return self._contains_bs_slow(item)  # slow version of binary-search (slicing)
    ...
    def __contains_bs_slow(self, L, item):
        """
        searches L for item.
        This is slow since it slices L at every level of recursion
        """
    ...
    def _bs(self, item, left: int, right: int) -> bool:
        ...
        if self[mid_idx] == item:
            return True

        # item is in smaller half
        if self[mid_idx] > item:
            return self._bs(item, left=left, right=mid_idx - 1)

        # item is in bigger half
        return self._bs(item, left=mid_idx + 1, right=right)
